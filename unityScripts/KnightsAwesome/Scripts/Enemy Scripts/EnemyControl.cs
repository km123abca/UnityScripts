...
public enum EnemyState{
	IDLE,
	WALK,RUN,PAUSE,GOBACK,ATTACK,DEATH
}
public class EnemyControl : MonoBehaviour {
	...
	EnemyState enemy_LastState=EnemyState.IDLE; 
	...
	void announceState(string strr)
		{
			if(strr==lastStateString)
				return;
			lastStateString=strr;
			cnter+=1;
			Debug.Log(cnter+": Present State:"+strr);
		}
	...
	EnemyState SetEnemyState(EnemyState curState,EnemyState lastState,float enemyToPlayerDis)		
		{
			...
			if(initialDistance > followDistance)
			...
			else if(enemyToPlayerDis <= attack_Distance)
				{
					...
					enemyToPlayerDis>=alert_Attack_Distance && lastState==EnemyState.PAUSE 
					|| 
	...
	void GetStateControl(EnemyState curState)
		{
			if(curState == EnemyState.RUN || curState==EnemyState.PAUSE)
				...
				{
					announceState("running back");
					anim.SetBool("Run",true);
					Vector3 targetPosition=new Vector3(initialPosition.x,
													   transform.position.y,
													   initialPosition.z
													  );
					navAgent.SetDestination(targetPosition);
					if(Vector3.Distance(targetPosition,transform.position)<=3.5f) //doubtful
						{
							Debug.Log("Patrolling is back");
