...
public class CharacterMovement : MonoBehaviour 
	...
	float runningForAWhileTimer=0f;
	public float maxRunningForAWhile=2f;
	...
	public float RunningForAWhileTimer
		{
			get { return runningForAWhileTimer;}
			set { runningForAWhileTimer=value;}
		}
	...
	void MovementAndJumping()
		{
			...
			if(Input.GetKeyDown(KeyCode.Q)) 
				moveInput=Quaternion.AngleAxis(90,Vector3.up)*transform.forward*10f;
			*/
			
			//Animating the player based on Input
			if(Mathf.Abs(Input.GetAxis("Vertical"))>0 || Mathf.Abs(Input.GetAxis("Horizontal"))>0)
				{
				runningForAWhileTimer+=Time.deltaTime;
				anim.SetFloat("Walk",1.0f);
				}
			else
				{
				runningForAWhileTimer=0f;
